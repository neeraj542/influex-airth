...
const HeroSection = () => {
  ...
  const handleInstagramAuth = () => {
    ...
    initiateInstagramAuth()
      .then((response) => {
        const { longLivedToken, lambdaResponse } = response.data;
        setAccessToken(longLivedToken.access_token);
        setLambdaResponse(lambdaResponse);
        localStorage.setItem('access_token', longLivedToken.access_token);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    if (authCode) {
      setLoading(true);
      initiateInstagramAuth(authCode)
        .then((response) => {
          const { longLivedToken, lambdaResponse } = response.data;
          setAccessToken(longLivedToken.access_token);
          setLambdaResponse(lambdaResponse);
